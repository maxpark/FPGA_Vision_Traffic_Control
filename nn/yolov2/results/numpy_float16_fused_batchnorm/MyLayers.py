...
class MyModel:
    ...
    def change_apply(self,func):
        for name in self.d.keys():
            self.d[name].apply = func
...
class MyConv(MyLayer):
    ...
    * Optionally allows fused Batch Normalization
    ...
    def __init__(self, 
        ...
        self.weights = self.apply(weights_biases[0])
        ...
        if len(weights_biases) > 1:
            self.biases = self.apply(weights_biases[1])
        else:
            self.biases = self.apply(np.zeros((self.out_ch_n), dtype = self.np_dtype))
    ...
    def np_out(self,in_data):
        ...
        if self.np_dtype == np.float16:
            in_data = self.apply(in_data)
            out = self.apply(out)
            self.weights_flipped = self.apply(self.weights_flipped)
            self.biases = self.apply(self.biases)
        
        self.np_out_data = self.apply(out)
    ...
    def fuse_bn(self, bn_weights, epsilon = 0.001):
        ...
        self.weights = self.apply(self.weights * scale)
        self.weights_flipped = np.flip(self.weights, [0,1])
        self.biases = self.apply(beta + scale * (self.biases - self.mean))
...
class MyInput(MyLayer):
    ...
    def get_np_output_recursively(self):
        return self.apply(self.input_image)
    def get_keras_output_recursively(self):
...
class MyLeakyRelu(MyLayer):
    ...
    def np_out(self, in_data):
        x = in_data
        self.np_out_data = self.apply(x* ((x > 0) + (x < 0) * self.alpha))
...
class MyMaxPool(MyLayer):
    ...
    def np_out(self,in_data):
        ...
        self.np_out_data = self.apply(self.np_out_data.max(axis=2).max(axis=3))
